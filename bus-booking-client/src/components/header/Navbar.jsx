...
export default function Navbar() {
  ...
  return (
    <header 
      className={`${Theme ? 'bg-white' : 'bg-gray-900'} shadow-md sm:px-8 px-4 ${
        isOpen && 'overflow-y-hidden md:min-h-fit min-h-screen'
      } fixed top-0 left-0 min-w-full z-50`}
    >
            ...
            >
              ...
              >
                {NavigationLinks.map((navigation, index) => (
                  <Link key={`${navigation.ID}${index}`} to={navigation.URL}>
                    <li
                      className={`block py-2 px-3 ${
                        location.pathname === navigation.URL
                          ...
                          : Theme
                          ? 'text-gray-900 rounded hover:bg-gray-100 md:hover:text-blue-700 md:hover:bg-transparent md:border-0'
                          : 'md:hover:text-blue-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent text-white md:border-0'
